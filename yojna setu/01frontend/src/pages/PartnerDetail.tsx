...
export const PartnerDetail: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      {/* Header */}
      <div className="bg-white p-6 rounded-2xl border border-[#E8D8D2] shadow-warm-xs flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <ApplicationStatusBadge status={appData.status} />
            <span className="text-xs font-mono text-[#765E59] font-bold">{t('partner.appId', 'App ID: {{id}}', { id: appData.application_id })}</span>
          </div>
          <h1 className="text-2xl font-extrabold text-[#3B2522]">{appData.scheme_name || `Scheme ${appData.scheme_id}`}</h1>
          <p className="text-xs text-[#765E59] mt-1">{t('partner.beneficiaryUserId', 'Beneficiary User ID: {{id}}', { id: appData.user_id })}</p>
        </div>
        ...
        {appData.status === 'APPROVED' && isDecisionMaker && (
          <button
            ...
            className="bg-emerald-700 hover:bg-emerald-800 text-white font-bold text-xs px-5 py-2.5 rounded-xl shadow-warm-xs transition flex items-center gap-1.5"
          >
        ...
        {!isFinalized && (
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="bg-[#F7AE56] hover:bg-[#e09840] text-[#4A2525] font-bold text-xs px-4 py-2.5 rounded-xl shadow-warm-xs transition flex items-center gap-1.5"
            >
            ...
            {isDecisionMaker && (
              <>
                <button
                  ...
                  className="bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs px-4 py-2.5 rounded-xl shadow-warm-xs transition flex items-center gap-1.5"
                >
                ...
                <button
                  ...
                  className="bg-[#EA717B] hover:bg-[#d65f69] text-white font-bold text-xs px-5 py-2.5 rounded-xl shadow-warm-xs transition flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
                >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="bg-white p-6 rounded-2xl border border-[#E8D8D2] shadow-warm-xs space-y-6">
            <h3 className="text-base font-bold text-[#3B2522] flex items-center gap-2 border-b border-[#E8D8D2] pb-3">
              <FileCheck2 className="w-5 h-5 text-[#F7AE56]" />
              {t('partner.docQueueTitle', 'Document Verification Queue ({{count}})', { count: appData.documents.length })}
            </h3>

            <div className="space-y-4">
              {appData.documents.map((doc) => (
                <div key={doc.app_document_id} className="bg-[#FFFBF0] p-4 rounded-xl border border-[#E8D8D2] space-y-3">
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="font-bold text-sm text-[#3B2522]">{doc.document_name}</h4>
                      <p className="text-xs text-[#765E59]">{doc.requirement_type} {t('partner.document', 'document')}</p>
                    </div>
                    ...
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-lg border uppercase ${
                      doc.verification_status === 'VERIFIED'
                        ...
                        : 'bg-[#FFF4EC] text-[#4A2525] border-[#FFD0CA]'
                    }`}>
                  ...
                  {doc.is_uploaded ? (
                    <div className="text-xs text-[#3B2522] bg-white p-2.5 rounded-xl border border-[#E8D8D2] flex justify-between items-center shadow-warm-xs">
                      <span>{t('applications.fileLabel', 'File:')} <strong>{doc.file_name}</strong></span>
                      <span className="text-[11px] text-[#765E59]">{t('partner.uploadedOn', 'Uploaded')} {doc.uploaded_at ? new Date(doc.uploaded_at).toLocaleDateString() : ''}</span>
                    </div>
                  ) : (
                  ...
                  {!isFinalized && doc.is_uploaded && (
                    <div className="pt-2 border-t border-[#E8D8D2] flex justify-end gap-2 text-xs">
                      <button
                        onClick={() => setRejectingDoc(doc)}
                        className="bg-rose-50 hover:bg-rose-100 text-rose-800 px-3 py-1.5 rounded-xl border border-rose-300 font-bold transition"
                      >
                      ...
                      <button
                        ...
                        className="bg-emerald-600 hover:bg-emerald-700 text-white px-3.5 py-1.5 rounded-xl font-bold shadow-warm-xs transition flex items-center gap-1"
                      >
          ...
          <div className="bg-white p-6 rounded-2xl border border-[#E8D8D2] shadow-warm-xs space-y-4">
            <h3 className="text-base font-bold text-[#3B2522] flex items-center gap-2 border-b border-[#E8D8D2] pb-3">
              <MessageSquare className="w-5 h-5 text-[#EA717B]" />
            ...
            {appData.review_notes && appData.review_notes.length > 0 && (
              <div className="space-y-3">
                {appData.review_notes.map((note) => (
                  <div key={note.note_id} className="bg-[#FFFBF0] p-3 rounded-xl border border-[#E8D8D2] text-xs">
                    <div className="flex justify-between items-center text-[#765E59] mb-1">
                      <span className="font-bold text-[#3B2522]">{note.author_role} ({note.author_id})</span>
                      <span>{new Date(note.created_at).toLocaleString()}</span>
                    </div>
                    <p className="text-[#3B2522]">{note.content}</p>
                  </div>
                ))}
              </div>
            )}
            ...
            {!isFinalized && (
              <form onSubmit={handleAddNote} className="space-y-2 pt-2">
                <textarea
                  ...
                  className="w-full p-3 rounded-xl border border-[#E8D8D2] bg-white text-[#3B2522] text-xs focus:ring-2 focus:ring-[#EA717B]/20 focus:border-[#EA717B] outline-none"
                  rows={3}
                />
                <button
                  type="submit"
                  disabled={!noteText}
                  className="bg-[#4A2525] hover:bg-[#3B2522] text-white font-bold text-xs px-4 py-2 rounded-xl shadow-warm-xs transition disabled:opacity-50"
                >
