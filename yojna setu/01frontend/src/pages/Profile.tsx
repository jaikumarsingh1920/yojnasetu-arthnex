...
export const Profile: React.FC = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      {/* ── Top Header Banner ── */}
      <div className="bg-gradient-to-r from-[#4A2525] via-[#3B2522] to-[#4A2525] text-white p-6 sm:p-8 rounded-3xl shadow-warm-md border border-[#E8D8D2]/20 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="space-y-2">
          <div className="inline-flex items-center gap-2 bg-[#FFF4EC]/15 text-[#FFD0CA] text-xs font-bold px-3 py-1 rounded-full border border-[#FFD0CA]/20">
          ...
          <p className="text-[#FFD0CA] text-sm max-w-2xl leading-relaxed">
            {t(
              'profile.subtitle',
              'Your structured profile acts as the single source of truth for deterministic eligibility evaluation across verified central and state government schemes. Zero document uploads required.'
            )}
          </p>
        </div>
        ...
        <Alert type={feedback.type}>
          <div className="flex items-center justify-between">
            ...
            {feedback.type === 'success' && (
              <Link to="/recommendations" className="text-xs font-bold underline ml-4 hover:opacity-80 text-[#EA717B]">
      ...
      <div className="bg-white rounded-2xl border border-[#E8D8D2] shadow-warm-xs p-6 flex flex-col md:flex-row items-center gap-6 justify-between">
        <div className="flex items-center gap-4">
          <div className="relative w-16 h-16 flex items-center justify-center">
            <svg className="w-16 h-16 transform -rotate-90" viewBox="0 0 36 36" aria-hidden="true">
              <path
                className="text-[#FFF4EC]"
                strokeWidth="3.5"
                stroke="currentColor"
                fill="none"
              ...
              />
              <path
                className={completionScore >= 80 ? 'text-emerald-500' : completionScore >= 50 ? 'text-[#F7AE56]' : 'text-[#EA717B]'}
            ...
            <span className="absolute text-sm font-black text-[#3B2522]">{completionScore}%</span>
          </div>

          <div>
            <h3 className="text-base font-bold text-[#3B2522]">
              {t('profile.completionTitle', 'Profile Readiness Score')}
            </h3>
            <p className="text-xs text-[#765E59]">
              {completionScore >= 80
                ? t('profile.completeReady', 'Comprehensive profile — all schemes can be evaluated accurately.')
        ...
        {missingFields.length > 0 && (
          <div className="flex flex-wrap gap-2 items-center justify-end">
            <span className="text-xs font-semibold text-[#765E59]">{t('profile.missingFields', 'Missing Information')}:</span>
            {missingFields.slice(0, 4).map((mf) => (
              <span
                key={mf.field}
                className="inline-flex items-center gap-1 text-[11px] font-bold bg-[#FFF4EC] text-[#4A2525] border border-[#FFD0CA] px-2.5 py-1 rounded-full shadow-warm-xs"
                title={mf.impact_reason}
              >
                <AlertTriangle className="w-3 h-3 text-[#EA717B]" />
                {mf.label}
              </span>
            ))}
            {missingFields.length > 4 && (
              <span className="text-[11px] font-medium text-[#765E59]">+{missingFields.length - 4} more</span>
            )}
          </div>
        )}
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-8">
        ...
        <div className="bg-white rounded-2xl border border-[#E8D8D2] shadow-warm-xs p-6 sm:p-8 space-y-6">
          <div className="flex items-center gap-3 border-b border-[#E8D8D2] pb-4">
            <div className="w-10 h-10 rounded-xl bg-[#FFF4EC] text-[#EA717B] flex items-center justify-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {/* Age */}
            <div>
              <label className="block text-xs font-bold text-[#3B2522] uppercase tracking-wider mb-2">
              ...
              <input
                ...
                placeholder="e.g. 28"
                className="w-full bg-[#FFFBF0] border border-[#E8D8D2] focus:border-[#EA717B] focus:ring-2 focus:ring-[#EA717B]/20 focus:bg-white rounded-xl px-4 py-2.5 text-sm text-[#3B2522] font-medium outline-none transition"
                required
              />
            </div>
            ...
            <div className="sm:col-span-2 lg:col-span-3 bg-[#FFFBF0] p-4 rounded-xl border border-[#E8D8D2]">
              <span className="block text-xs font-bold text-[#3B2522] uppercase tracking-wider mb-3">
                {t('profile.specialAffirmations', 'Special Beneficiary Affirmations')}
              </span>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                <label className="flex items-center gap-2.5 cursor-pointer text-xs font-semibold text-[#3B2522]">
                  <input
                    type="checkbox"
                    checked={!!profileData.is_pwd}
                    onChange={(e) => handleChange('is_pwd', e.target.checked)}
                    className="w-4 h-4 accent-[#EA717B] rounded border-[#E8D8D2]"
                  />
        ...
        <div className="bg-white rounded-2xl border border-[#E8D8D2] shadow-warm-xs p-6 sm:p-8 space-y-6">
          <div className="flex items-center gap-3 border-b border-[#E8D8D2] pb-4">
            <div className="w-10 h-10 rounded-xl bg-[#FFF4EC] text-[#F7AE56] flex items-center justify-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="relative">
                <span className="absolute left-4 top-2.5 text-[#765E59] font-bold text-sm">₹</span>
                <input
                  type="number"
                  min={0}
                  step={1000}
                  ...
                  className="w-full bg-[#FFFBF0] border border-[#E8D8D2] focus:border-[#EA717B] focus:ring-2 focus:ring-[#EA717B]/20 focus:bg-white rounded-xl pl-8 pr-4 py-2.5 text-sm text-[#3B2522] font-medium outline-none transition"
                  required
                />
              </div>
              <p className="text-[11px] text-[#765E59] mt-1">
        ...
        <FinancialHealthCard profileData={profileData} isAuthenticated={isAuthenticated} />

        {/* ── Privacy & Zero-Document Guarantee Box ── */}
        <div className="bg-[#FFF4EC] border border-[#FFD0CA] rounded-2xl p-5 flex items-start gap-4 text-[#4A2525]">
          <Lock className="w-5 h-5 text-[#EA717B] flex-shrink-0 mt-0.5" />
          <div className="text-xs space-y-1">
            <h4 className="font-bold text-[#3B2522]">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-4 pt-4 border-t border-[#E8D8D2]">
          <button
            type="button"
            onClick={() => loadProfile()}
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl border border-[#E8D8D2] bg-white hover:bg-[#FFFBF0] text-[#765E59] font-bold text-sm transition shadow-warm-xs min-h-[44px]"
          >
            <RefreshCw className="w-4 h-4 text-[#765E59]" />
          ...
          <button
            type="submit"
            disabled={isSaving}
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-[#EA717B] hover:bg-[#d65f69] text-white font-bold text-sm transition shadow-warm-xs min-h-[44px]"
          >
