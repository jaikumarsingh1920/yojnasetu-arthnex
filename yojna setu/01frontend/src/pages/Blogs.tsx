  ...
  return <div className="min-h-screen overflow-hidden bg-[#f8f6f1] text-slate-800">
    <section className="relative isolate overflow-hidden bg-[#071b2b] text-white">
      ...
      <div className="blog-enter relative mx-auto flex min-h-[400px] max-w-7xl flex-col justify-center px-5 py-16 sm:px-8 sm:py-20 lg:px-10">
        <div className="inline-flex w-fit items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3.5 py-2 text-[11px] font-extrabold tracking-[.16em] text-amber-200"><Sparkles className="h-3.5 w-3.5" /> YOJNASETU FINANCIAL JOURNAL</div>
        <h1 className="mt-6 max-w-3xl text-4xl font-black leading-[1.08] tracking-tight sm:text-6xl">Money clarity for <span className="text-amber-300">everyday India.</span></h1>
        <p className="mt-5 max-w-2xl text-base leading-7 text-white/75 sm:text-lg">Thoughtful explainers on savings, credit, loans and government financial support—written to make every next step feel simpler.</p>
        <div className="mt-8 flex flex-wrap items-center gap-3 text-sm font-semibold text-white/85"><span className="inline-flex items-center gap-2"><TrendingUp className="h-4 w-4 text-amber-300" /> Practical financial literacy</span><span className="h-1 w-1 rounded-full bg-white/50" /><span>{blogs.length} published {blogs.length === 1 ? 'story' : 'stories'}</span></div>
      </div>
    </section>
    <section className="max-w-7xl mx-auto px-5 sm:px-8 py-12 lg:px-10">
      <div className="flex items-center justify-between gap-4 mb-7">
        <p className="text-sm text-slate-600">Financial information published by YojnaSetu.</p>
        {isAdmin && <button onClick={() => openEditor()} className="inline-flex items-center gap-2 rounded-xl bg-[#861823] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#6f1420]"><Plus className="w-4 h-4" /> New blog</button>}
      </div>
      {error && <div className="mb-5 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error}</div>}
      {loading ? <p className="py-12 text-center text-slate-500">Loading financial blogs…</p> : blogs.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center text-slate-500">No financial blogs have been published yet.</div> : <div className="grid gap-5 md:grid-cols-2">
        {blogs.map(blog => <article key={blog.blog_id} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex justify-between gap-3"><h2 className="text-xl font-extrabold text-slate-900">{blog.title}</h2>{isAdmin && <div className="flex shrink-0 gap-1"><button aria-label="Edit blog" onClick={() => openEditor(blog)} className="rounded-lg p-2 text-slate-600 hover:bg-slate-100"><Pencil className="w-4 h-4" /></button><button aria-label="Delete blog" onClick={() => remove(blog)} className="rounded-lg p-2 text-red-600 hover:bg-red-50"><Trash2 className="w-4 h-4" /></button></div>}</div>
          <p className="mt-3 text-sm font-medium text-slate-600">{blog.summary}</p><p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-slate-700">{blog.content}</p>
          <div className="mt-5 flex items-center gap-2 border-t pt-4 text-xs text-slate-500"><CalendarDays className="w-4 h-4" /> {new Date(blog.updated_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })} · {blog.author_name}</div>
        </article>)}
      </div>}
    </section>
