  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 flex items-center gap-2">
            ...
            Assisted Application Guidance & Document Checklists
          </h1>
          <p className="text-xs text-slate-500 mt-1 max-w-2xl leading-relaxed">
            YojnaSetu helps you discover schemes, review eligibility, and prepare your document checklists. Final application submission and approval are handled directly on the concerned official government portal.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            <option value="">All Guidance Records</option>
            <option value="DRAFT">Guidance Started</option>
            <option value="DOCUMENTS_PENDING">Checklist Pending</option>
            <option value="READY_FOR_SUBMISSION">Ready to Apply on Official Portal</option>
            <option value="SUBMITTED">Redirected to Official Portal</option>
            <option value="UNDER_REVIEW">Assisted Review</option>
            <option value="APPROVED">Guidance Completed</option>
            <option value="REJECTED">Criteria Unmet</option>
          </select>
      ...
      <div className="bg-sky-50 border border-sky-200 p-4 rounded-2xl flex items-start gap-3 text-xs text-sky-900">
        ...
        <div>
          <p className="font-bold">Official Application Portal Reminder</p>
          <p className="text-[11px] text-sky-800 mt-0.5">
            Once you complete your document checklist on YojnaSetu, use the reference link to submit your official application on the concerned government portal. Keep your official reference/acknowledgement number for tracking.
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="py-16 text-center">
          ...
          <p className="text-xs text-slate-500 mt-2 font-medium">Loading application guidance records...</p>
        </div>
      ) : applications.length === 0 ? (
        <div className="py-16 text-center bg-white rounded-2xl border border-slate-200">
          <FileText className="w-12 h-12 text-slate-300 mx-auto mb-3" />
          <h3 className="text-base font-bold text-slate-800">No Guidance Records Found</h3>
          <p className="text-xs text-slate-500 max-w-md mx-auto mt-1">
            You don't have any application records matching the selected status filter.
          </p>
        </div>
      ) : (
              ...
              <div key={app.application_id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-slate-500 font-mono">
                    Guidance ID: {app.application_id} | Created: {new Date(app.created_at).toLocaleDateString()}
                  </p>

                  <div className="flex flex-wrap items-center gap-4 text-xs text-slate-600 pt-1">
                    <span>Document Checklist: <strong>{uploadedDocs} / {totalDocs} items checked</strong></span>
                  </div>
                </div>
                ...
                >
                  View Checklist & Official Portal Link
