...
export const Applications: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      {/* Header */}
      <div className="bg-white p-6 sm:p-8 rounded-2xl border border-[#E8D8D2] shadow-warm-xs flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-[#3B2522] flex items-center gap-2">
            <FileText className="w-6 h-6 text-[#EA717B]" />
            {t('applications.guidanceTitle', 'Assisted Application Guidance & Document Checklists')}
          </h1>
          <p className="text-xs text-[#765E59] mt-1 max-w-2xl leading-relaxed">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <select
            ...
            className="px-3.5 py-2.5 rounded-xl border border-[#E8D8D2] text-xs focus:ring-2 focus:ring-[#EA717B]/20 focus:border-[#EA717B] outline-none bg-white text-[#3B2522] font-medium"
          >
      ...
      {isLoading ? (
        <div className="py-16 text-center">
          ...
          <p className="text-xs text-[#765E59] mt-2 font-medium">{t('applications.loadingRecords', 'Loading application guidance records...')}</p>
        </div>
      ) : applications.length === 0 ? (
        <div className="py-16 text-center bg-white rounded-2xl border border-[#E8D8D2]">
          <FileText className="w-12 h-12 text-[#765E59]/40 mx-auto mb-3" />
          <h3 className="text-base font-bold text-[#3B2522]">{t('applications.noRecordsFound', 'No Guidance Records Found')}</h3>
        ...
        <div className="space-y-4">
          {applications.map((app) => {
            ...
            return (
              <div key={app.application_id} className="bg-white p-4 sm:p-6 rounded-2xl border border-[#E8D8D2] shadow-warm-xs hover:shadow-warm-md transition flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <Link
                  ...
                  className="w-full sm:w-auto bg-[#EA717B] hover:bg-[#d65f69] text-white text-xs font-bold px-4 py-2.5 rounded-xl shadow-warm-xs transition flex items-center justify-center gap-1.5 shrink-0 min-h-[44px]"
                >
